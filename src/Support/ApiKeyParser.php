...
class ApiKeyParser
{
     ...
     * - New format: lh_proj_{project_id}_{base64_webhook_secret}
     * - Legacy format: any string (no embedded secret)
     */
    public static function parse(string $apiKey): array
    {
        ...
        if (Str::startsWith($apiKey, 'lh_proj_')) {
            ...
            if (count($parts) === 3) {
                // Format without webhook secret: lh_proj_{id}
                return [
            ...
            // Format with webhook secret: lh_proj_{id}_{secret}
            $projectId = $parts[2];
        ...
        // Legacy format: plain API key without embedded secret
        return [
            'project_id' => $apiKey,
            'webhook_secret' => null,
        ];
